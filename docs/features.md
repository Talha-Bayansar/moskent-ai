...
| --- | --- | --- |
| Organization and user foundation | in progress | Better Auth backend foundation now includes email/password, the organization plugin, dynamic per-organization roles, a server-backed current-user query with active organization and active role permissions, query-owned organization state, organization creation at `/organizations/new`, an organization access hub at `/organizations`, a dedicated no-org invitations page at `/organizations/invitations`, a dashboard invitations page at `/dashboard/invitations`, authenticated organization bootstrap, a shared organization-access shell for `/organizations` and `/organizations/invitations`, shell-aware organization creation at `/organizations/new`, a dashboard organization switcher with a create-organization action, an authenticated profile menu with reusable sign-out confirmation, a dashboard settings page for user preferences, a members browser at `/dashboard/members`, and a reusable invite flow at `/dashboard/members/invite`. Pending invitation cards now prioritize organization names and avoid showing raw organization IDs, accepted invitations now move users into `/dashboard`, post-auth landing is membership-aware, and auth transitions clear and rehydrate the current-user and organization query caches. Broader organization management workflows and UI are still `TBD`. |
