...
| --- | --- | --- |
| Organization and user foundation | in progress | Better Auth backend foundation now includes email/password, the organization plugin, dynamic per-organization roles, organization creation at `/organizations/new`, authenticated organization bootstrap, a dashboard organization switcher, an authenticated profile menu with reusable sign-out confirmation, a dashboard settings page for user preferences, a members browser at `/dashboard/members`, and a reusable invite flow at `/dashboard/members/invite`. Broader organization management workflows and UI are still `TBD`. |
